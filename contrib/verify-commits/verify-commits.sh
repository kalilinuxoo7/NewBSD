...
while true; do
	...
	if [ "$NO_SHA1" = "1" ]; then
		export BITSEND_VERIFY_COMMITS_ALLOW_SHA1=0
	else
		export BITSEND_VERIFY_COMMITS_ALLOW_SHA1=1
	fi

	if [ "${REVSIG_ALLOWED#*$CURRENT_COMMIT}" != "$REVSIG_ALLOWED" ]; then
		export BITSEND_VERIFY_COMMITS_ALLOW_REVSIG=1
	else
		export BITSEND_VERIFY_COMMITS_ALLOW_REVSIG=0
	fi
