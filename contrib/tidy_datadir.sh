...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Bitsend database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Bitsend datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Bitsend datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Bitsend 0.7 datadir."
    ;;
  3)
    echo "Detected Bitsend pre-0.8 datadir."
    ;;
  4)
    echo "Detected Bitsend 0.8 datadir."
    ;;
esac
