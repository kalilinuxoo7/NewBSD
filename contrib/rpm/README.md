...
reference the release tarball available from https://bitsend.org/bin/ and
...
Currently only `Source10` is used, for the example `bitsend.conf` file.
...
necessary, the packager should bring the problem to the attention of the bitsend
...
matters for the `bitsend-server` package. At this point, most RPM based
...
The `bitsend-server` package should have SELinux support. How to properly do
...
4. I renamed the `bitsend` package that contains the Qt GUI to `bitsend-core` as
that appears to be how the general population refers to it, in contrast to
`bitsend-xt` or `bitsend-classic`. I wanted to make sure the general population
