Bitsend Core version 0.13.1 is now available from:

  <https://bitsend.org/bin/bitsend-core-0.13.1/>
...
wallet on a XP bitsend is irresponsible at least.
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Bitsend wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Bitsend.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Bitsend block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of bitsends they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of bitsends being spent is exactly
...
- **Increased security for multisig:** Bitsend addresses (both P2PKH addresses
  ...
  believed to be provided by Bitsend's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Bitsend paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Bitsend's current ledger (called the UTXO set),
  ...
  note that Bitsend Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Bitsend users to individually opt-in to almost any change in the Bitsend
  Script language when those users receive new transactions.  Features
  currently being researched by Bitsend Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Bitsend community members are studying several other improvements
...
[Segwit FAQ]: https://bitsend.io/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://bitsend.io/en/segwit_wallet_dev/
[BIP141]: https://github.com/bitsend/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/bitsend/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/bitsend/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/bitsend/bips/blob/master/bip-0145.mediawiki
[versionbits FAQ]: https://bitsend.io/en/2016/06/08/version-bits-miners-faq/
...
Since Bitsend Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/bitsend/bips/blob/master/bip-0147.mediawiki
...
- `bitsend-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `bitsend-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in bitsendconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.bitsendstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libbitsendconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_bitsend by itself (luke-jr)
...
- #8829 `2a8bca4` Add bitsend-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` bitsend-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_bitsendds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name bitsend.qrc  (bitsendsSG)
...
- #8817 `bcf3806` update bitsend-tx to output witness data (jnewbery)
